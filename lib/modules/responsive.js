...
const debug = debugModule('analyze-image:sizesAttribute');

import * as htmlparser from 'htmlparser2';
import {parseSrcset, stringifySrcset} from 'srcset';
import * as mediaQueryParser from 'media-query-fns';
import {transform as cssCalcTransform} from 'css-calc-transform';
...
export function findSizesAndSrcsetAttributes(html, viewportWidth, viewportHeight, dpr) {
...
export function validateSrcset(str) {
  if (!str) {
    return false;
  }

  debug('Parsing the srcset attribute %s', str);
  const parsed = parseSrcset(str);
...
export function matchMediaQuery(str, viewportWidth, viewportHeight, dpr) {
  if (!str) {
    return false;
  }
...
export function computeSizeFromAttribute(sizesAttr, viewportWidth, viewportHeight, dpr) {
...
export function computeOneSizeValue(sizeValue, viewportWidth, viewportHeight) {
  ...
  try {
    const pxValue = cssCalcTransform({
      prop: 'width',
      value: calc,
      win: {
        width: viewportWidth,
...
export default main;
export const description = 'Checks the sizes attribute of an HTML <img> or <picture>';
