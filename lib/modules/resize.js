...
const debug = debugModule('analyze-image:resize');
...
async function main(runner) {
  ...
  if (sizeRatio < 1) {
    ...
    if (sizeRatio > RESIZE_IF_RATIO_MORE_THAN) {
      ...
      newFile = await optimizeImage(newFile, runner.result.stats.format, runner.options);
  ...
  if (runner.result.stats.displayDensity > MAX_DISPLAY_DENSITY * 1.1 && (dpr || 1) >= MAX_DISPLAY_DENSITY * 1.1) {
    ...
    let referenceDensity = runner.result.stats.displayDensity;
    ...
    if (newFile && newFile.length > 0) {
      ...
      if (runner.result.offenders.imageScaledDown) {
        referenceWeight = runner.result.transforms.resized.newFileSize;
        referenceDensity = dpr;
        alreadyResized = true;
      }
...
export default main;
export const description = 'Checks if image can be resized, skipping.';
