...
import {
    validateSrcset,
    findSizesAndSrcsetAttributes,
    matchMediaQuery,
    computeOneSizeValue,
    computeSizeFromAttribute
} from '../lib/modules/responsive.js';
import assert from 'assert';


describe('Sizes HTML attribute checker', () => {
    it('should fail if no HTML code is provided', async () => {
        const result = await execModuleForTest('responsive', '', {
            displayWidth: 100,
            displayHeight: 100,
            viewportWidth: 1200,
            viewportHeight: 800
...
describe('parse a srcset attribute and check that it is a valid w type', () => {
    it('succeeds with a single url', async () => {
        const validated = validateSrcset('image1.jpg 1w');
        assert.strictEqual(validated, 'image1.jpg 1w');
    });
    it('succeeds with 2 urls', async () => {
        const validated = validateSrcset(' image1.jpg 1w ,  image2.jpg 2w ');
        assert.strictEqual(validated, 'image1.jpg 1w, image2.jpg 2w');
    });
    it('succeeds with 3 urls', async () => {
        const validated = validateSrcset('image1.jpg  1w, image2.jpg 2w  , image3.jpg 3w,');
        assert.strictEqual(validated, 'image1.jpg 1w, image2.jpg 2w, image3.jpg 3w');
    });
    it('fails without w', async () => {
        const validated = validateSrcset('image1.jpg 1');
        assert.strictEqual(validated, false);
    });
    it('fails without w', async () => {
        const validated = validateSrcset('image1.jpg');
        assert.strictEqual(validated, false);
    });
    it('fails with x url', async () => {
        const validated = validateSrcset('image1.jpg 1x');
        assert.strictEqual(validated, false);
    });
    it('fails with multiple x url', async () => {
        const validated = validateSrcset('image1.jpg 1x, image2.jpg 2x');
        assert.strictEqual(validated, false);
    });
    it('fails with mixed urls', async () => {
        const validated = validateSrcset('image1.jpg 1x, image2.jpg 2w');
        assert.strictEqual(validated, false);
    });
    it('fails with mixed urls', async () => {
        const validated = validateSrcset('image1.jpg 1w, image2.jpg 2x');
        assert.strictEqual(validated, false);
    });
    it('fails if empty', async () => {
        const validated = validateSrcset('');
        assert.strictEqual(validated, false);
    });
    it('fails if no x and no w', async () => {
        const validated = validateSrcset('image.jpg');
        assert.strictEqual(validated, false);
    });
    it('fails if srcset is undefined', async () => {
        const validated = validateSrcset(undefined);
        assert.strictEqual(validated, false);
    });
});
...
describe('Look for sizes and srcset attributes on a simple <img> tag', () => {
    it('should not find sizes if there is no srcset', async () => {
        ...
        const {sizes, srcset} = findSizesAndSrcsetAttributes(html);
...
describe('Quickly checks the media query matcher', () => {
    it('should correctly check resolution x unit', async () => {
        const matches = matchMediaQuery('(min-resolution: 2.5x)', 300, 300, 2);
        assert.strictEqual(matches, false);
    });
    it('should correctly check resolution x unit', async () => {
        const matches = matchMediaQuery('(min-resolution: 2.5x)', 300, 300, 3);
        assert.strictEqual(matches, true);
    });
    it('should fail silently', async () => {
        const matches = matchMediaQuery('foo: 333px', 300, 300, 3);
        assert.strictEqual(matches, false);
    });
});
...
describe('Look for sizes and srcset attributes on a <picture> tag', () => {
    it('should not find sizes if there is no srcset', async () => {
        ...
        const {sizes, srcset} = findSizesAndSrcsetAttributes(html, 700, 700, 2);
        assert.strictEqual(sizes, undefined);
        assert.strictEqual(srcset, undefined);
    });
    it('should not find sizes if there is no srcset', async () => {
...
describe('Parse and match a sizes value', () => {
    it('should fail silently on an incorrect sizes attribute', async () => {
        const value = computeOneSizeValue('foo', 300, 700);
        assert.strictEqual(value, undefined);
    });
    it('should fail silently on an empty sizes attribute', async () => {
        const value = computeOneSizeValue('', 300, 700);
        assert.strictEqual(value, undefined);
    });
    it('parse correctly vw', async () => {
        const value = computeOneSizeValue('100vw', 300, 700);
        assert.strictEqual(value, 300);
    });
    it('parse correctly vw', async () => {
        const value = computeOneSizeValue('50vw', 300, 700);
        assert.strictEqual(value, 150);
    });
    it('parse correctly vh', async () => {
        const value = computeOneSizeValue('10vw', 300, 700);
    ...
    it('parse correctly calculate complicated stuff', async () => {
        const value = computeOneSizeValue('calc(100vw + (2 * 50vw) - 10em + 3px)', 300, 700);
        assert.strictEqual(value, 443);
    });
    it('work with decimal numbers', async () => {
        const value = computeOneSizeValue('calc(((100vw - 8.4vw) / 2) - 1.94vw)', 300, 700);
...
describe('Parse and match an entire sizes attribute', () => {
    it('should fail silently on an incorrect sizes attribute', async () => {
        const value = computeSizeFromAttribute('foo', 300, 700, 1);
        assert.strictEqual(value, undefined);
    });
    it('should fail silently on an empty sizes attribute', async () => {
        const value = computeSizeFromAttribute('', 300, 700, 1);
        assert.strictEqual(value, undefined);
    });
    it('parse a single value', async () => {
        const value = computeSizeFromAttribute('100vw', 300, 700, 1);
        assert.strictEqual(value, 300);
    });
    it('parse a simple value with calc', async () => {
        const value = computeSizeFromAttribute('calc(50vw + 3px)', 300, 700, 1);
        assert.strictEqual(value, 153);
    });
    it('parse multiple values and pick the right one', async () => {
        const value = computeSizeFromAttribute('(max-width: 400px) 10vw, 20vw', 300, 700, 1);
        assert.strictEqual(value, 30);
    });
    it('parse multiple values and pick the right one', async () => {
        const value = computeSizeFromAttribute('(max-width: 200px) 5vw, (max-width: 400px) 10vw, 20vw', 300, 700, 1);
        assert.strictEqual(value, 30);
    });
    it('pick correctly the default value when mediaqueries are not good', async () => {
        const value = computeSizeFromAttribute('(max-width: 100px) 5vw, (max-width: 200px) 6vw, 10vw', 300, 700, 1);
        assert.strictEqual(value, 30);
    });
    it('work with extra whitespaces', async () => {
        const value = computeSizeFromAttribute(' ( max-width : 100px )  5vw ,  ( max-width :  200px )   6vw, 10vw ', 300, 700, 1);
    ...
    it('work without whitespaces', async () => {
        const value = computeSizeFromAttribute('(max-width:100px) 5vw,(max-width:200px) 6vw,10vw', 300, 700, 2);
        assert.strictEqual(value, 30);
    });
});
...
describe('Detect incorrect sizes attributes from HTML', () => {
    it('doesn\'t detect anything on an image without srcset and sizes', async () => {
        const res = await execModuleForTest('responsive', '<svg></svg>', {
            displayWidth: 200,
            displayHeight: 100,
            viewportWidth: 300,
            viewportHeight: 700,
